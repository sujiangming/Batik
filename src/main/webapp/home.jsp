				...
				<div class="first_div">
					<div class="first_first_div">
						...
						<div class="add_window_content_top">
							<font id="addWinDesc">新增/编辑订单</font> <img id="close_window"
								src="img/close_btn.png" />
						</div>
						<!--表单上半部分内容-->
						<div style="width: 100%; height: auto; position: relative;">
							<div class="add_window_desc">
								<font>* 订单编号</font> <input id="goodsCode" type="text" />
							</div>
							<div class="add_window_desc">
								<font>* 订单名称</font> <input type="text" id="goodsName" />
							</div>
							<div class="add_window_desc">
								<font>* 订单价格</font> <input type="text" id="goodsPrice" />
							</div>
							<div class="add_window_desc">
								<font>* 食堂类型</font> <select id="goodsType" name="goodsType">
									<option value="师大一食堂">师大一食堂</option>
									<option value="师大二食堂">师大二食堂</option>
									<option value="师大三食堂">师大三食堂</option>
									<option value="思雅美食城">思雅美食城</option>
								</select>
							</div>
							<div class="add_window_desc">
								<font>* 订单简介</font> <input id="goodsDesc" name="goodsDesc"
									type="text" />
							</div>
...
<script type="text/javascript">
	...
	function edit_win(id) {
		...
		//根据订单id查询订单信息
		$.ajax({
			url : "http://localhost:8080/university/goods/queryById",
			type : "POST",
			data : json,
			dataType : "json",
	...
	function deleteGoodsById() {
		...
		//根据订单id查询订单信息
		$.ajax({
			url : "http://localhost:8080/university/goods/deleteById",
			type : "POST",
			data : json,
			dataType : "json",
			success : function(ret) {
	...
	function choose_image() {
		...
		$.ajax({
			url : 'http://localhost:8080/university/goods/uploadImage',
			type : 'POST',
			data : formData,
	...
	function addGoods() {
		...
		$.ajax({
			type : "POST",
			url : "http://localhost:8080/university/goods/save",
			data : json,
			dataType : "json",
	...
	function updateGoods() {
		...
		$.ajax({
			type : "POST",
			url : "http://localhost:8080/university/goods/update",
			data : json,
			dataType : "json",
			success : function(result) {
				if (result.status == 1) {
	...
	function initABData(){
		$.ajax({
			type : "GET",
			url : "http://localhost:8080/university/goods/initABTestData",
			dataType : "json",
			success : function(result) {
				if (result.status == 1) {
	...
	function dataGeneration(){
		$.ajax({
			type : "GET",
			url : "http://localhost:8080/university/goods/dataGeneration",
			dataType : "json",
			success : function(result) {
				console.log(JSON.stringify(result));
				if (result.status == 1) {
	...
	function dataAcquisition(){
		$.ajax({
			type : "GET",
			url : "http://localhost:8080/university/goods/dataAcquisition",
			dataType : "json",
			success : function(result) {
				console.log(JSON.stringify(result));
				if (result.status == 1) {
	...
	function dataStorage(){
		$.ajax({
			type : "GET",
			url : "http://localhost:8080/university/goods/dataStorage",
			dataType : "json",
			success : function(result) {
				console.log(JSON.stringify(result));
				if (result.status == 1) {
	...
	function dataAnalysis(){
		$.ajax({
			type : "GET",
			url : "http://locakhost:8080/university/goods/dataAnalysis",
			dataType : "json",
			success : function(result) {
				console.log(JSON.stringify(result));
				if (result.status == 1) {
